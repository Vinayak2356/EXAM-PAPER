...
const appCss = "/assets/styles-D6ZL29_O.css";
...
const Route$3 = createRootRouteWithContext()({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Professional Exam Paper, Homework, and Timetable Formatter" },
      { name: "author", content: "Vinayak" },
      { property: "og:title", content: "Exam Formatter Pro" },
      { property: "og:description", content: "Professional Exam Paper, Homework, and Timetable Formatter" },
...
const $$splitComponentImporter = () => import("./index-C6Q9E2xE.js");
const Route$2 = createFileRoute("/")({
  ...
  head: () => ({
    meta: [{
      ...
      name: "description",
      content: "Generate properly formatted exam papers and beautiful holiday homework packets — including a premium VVIP school template — from any text or website."
    }]
  })
});
async function fetchSiteText$1(url) {
  try {
    ...
    return html.replace(/<script[\s\S]*?<\/script>/gi, " ").replace(/<style[\s\S]*?<\/style>/gi, " ").replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim().slice(0, 3e3);
  } catch {
    return "";
  }
}
const SYSTEM_PROMPT$1 = `Indian-school holiday-homework setter. Output ONLY JSON:
{"title":string,"generalInstructions":string[],"subjects":[{"subject":string,"note":string|null,"assignments":[{"number":string,"text":string}]}]}
Rules: 4-6 varied tasks per subject. Premium tone = enrichment/research. Use Hindi script for Hindi/Sanskrit. Be concise.`;
const Route$1 = createFileRoute("/api/public/generate-hhw")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        const apiKey = process.env.VINAYAK_API_KEY;
        if (!apiKey) {
        ...
        const siteText = body.websiteUrl ? await fetchSiteText$1(body.websiteUrl) : "";
        const trimmedSource = (body.sourceText || "").slice(0, 3e3);
        const userPrompt = `Class:${body.className} Subjects:${body.subjects.join(",")} Tone:${body.tone === "premium" ? "premium" : "standard"} Title:${body.title}${body.instructions ? ` Note:${body.instructions}` : ""}
SOURCE:
${trimmedSource || "(none)"}${siteText ? `
SITE:${siteText}` : ""}`;
        const aiRes = await fetch(
          "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-lite:generateContent?key=" + apiKey,
          {
            ...
            body: JSON.stringify({
              contents: [
                { role: "user", parts: [{ text: SYSTEM_PROMPT$1 + "\\n\\n" + userPrompt }] }
              ],
              generationConfig: {
                temperature: 0.5,
...
async function fetchSiteText(url) {
  try {
    ...
    return cleaned.slice(0, 3e3);
  } catch {
    return "";
  }
}
const SYSTEM_PROMPT = `Indian school exam-paper setter. Output ONLY JSON:
{"sections":[{"name":string,"marks":number,"questions":[{"number":string,"instruction":string,"marks":number,"type":"mcq"|"short"|"long"|"fill"|"match"|"passage"|"other","passage":string|null,"subQuestions":[{"label":string,"text":string,"options":string[]|null}]}]}]}
Rules: section marks sum = Max Marks. MCQ = 4 options. Base questions on source. Be concise.`;
const Route2 = createFileRoute("/api/public/generate-exam")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        const apiKey = process.env.VINAYAK_API_KEY;
        ...
        const trimmedSource = (body.sourceText || "").slice(0, 4e3);
        const userPrompt = `Class:${body.className} Subject:${body.subject} MaxMarks:${body.maxMarks} Time:${body.time}${body.instructions ? ` Note:${body.instructions}` : ""}
SOURCE:
${trimmedSource || "(none)"}${siteText ? `
SITE:${siteText}` : ""}
Produce paper JSON totalling exactly ${body.maxMarks} marks.`;
        const aiRes = await fetch(
          "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-lite:generateContent?key=" + apiKey,
          {
            method: "POST",
...
const ApiPublicGenerateHhwRoute = Route$1.update({
  id: "/api/public/generate-hhw",
  path: "/api/public/generate-hhw",
  getParentRoute: () => Route$3
});
const ApiPublicGenerateExamRoute = Route2.update({
...
const rootRouteChildren = {
  IndexRoute,
  ApiPublicGenerateExamRoute,
  ApiPublicGenerateHhwRoute
};
